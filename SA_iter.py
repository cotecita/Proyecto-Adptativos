...
def egreedy(nodes_selected, choosen_dataset_path, filename, init_mat, end_mat):
...
if __name__ ==  "__main__":
    ...
    while density < 1:
        ...
        for f in os.listdir(directory):
            ...
            egreedy(nodes_selected, choosen_dataset_path, filename, init_matrix, final_matrix)
            best_solution = len(nodes_selected)
            current_solution = len(nodes_selected)
            best_time = time.time() - start_time
            print("Primera solución es " + str(len(nodes_selected)))
