...
def egreedy(f, init_mat, determinismo, size_lista):
...
def es_conjunto_independiente(nodos_seleccionados, matriz_adyacencia):
    for i in range(len(nodos_seleccionados)):
        for j in range(i + 1, len(nodos_seleccionados)):
            u = nodos_seleccionados[i]
            v = nodos_seleccionados[j]
            if matriz_adyacencia[u][v] > 0:
                return False  # Hay una conexión entre u y v
    return True  # Ningún par está conectado

def egreedy2(total_nodes, init_mat, determinismo, size_lista):
    nodes_selected = []
    graph_matrix = [row[:] for row in init_mat]
    ...
    def build_heap():
        return [(total_neighbors[i], i) for i in range(total_nodes) if not removed[i]]
    ...
    while heap:
        ...
        if random.random() < determinismo:
            candidatos = build_heap()
            candidatos.sort()
            k = min(size_lista, len(candidatos))
            min_index = random.choice([i for _, i in candidatos[:k]])
            #print("se seleccionó " + str(min_index) + " en sorteo al azar")
        else:
            heap = build_heap()
            _, min_index = heapq.heappop(heap)
            #print("se seleccionó " + str(min_index) + " porque tenía " + str(_) + " vecinos")
        ...
        for neighbor in range(total_nodes):
            if graph_matrix[min_index][neighbor] == 1 and not removed[neighbor]:
                #print("se eliminó el vecino " + str(neighbor))
        ...
        # Reconstruir heap con grados actualizados
        heap = build_heap()
        heapq.heapify(heap)
...
if __name__ ==  "__main__":
    ...
    while density < 1:
        ...
        for f in os.listdir(directory):
            ...
            density_best.append(best_solution)
            density_times.append(best_time)

        total_density_best = 0
        total_density_time = 0
        for solution in density_best:
            total_density_best = total_density_best + solution
        for times in density_times:
            total_density_time = total_density_time + times

        sol_mean = total_density_best/30

        print("PARA LA DENSIDAD p = " +  f"{density:.1f}")
        print("Media de mejores soluciones: " + str(sol_mean))

        #std deviation
        sum = 0
        for solution in density_best:
            sol_sum = math.pow(solution - sol_mean, 2)
            sum = sum + sol_sum
        sum = sum/30
        dv = math.sqrt(sum)

        print("Desviación estándar: " + str(dv))
        print("Tiempo total de todas las instancias: " + str(total_density_time))        
